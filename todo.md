...
**✉️ Email: FormSubmit ABANDONED → Resend.** Two FormSubmit bugs found: (1) it needs Origin/Referer headers or returns HTTP 200 + `success:"false"` (old `res.ok` check passed while nothing sent — that's why no email/activation ever fired); (2) even fixed, FormSubmit sits behind **Cloudflare, which 403s Vercel's datacenter IPs** (verified "Just a moment…" challenge) — so it can NEVER send from a Vercel function (works only from a browser/residential IP). Switched to **Resend** (`lib/email.ts`, Vercel Marketplace messaging): env-gated REST call, key+destination server-side, `from` = `onboarding@resend.dev` (no domain needed but only delivers to the Resend-account owner's address until a domain is verified). Route deployed; without the key `sent:false` and the DB still stores. **⚠️ blocked on user**: accept Resend marketplace terms (vercel.com → integrations → accept-terms/resend), then re-run `vercel integration add resend` (provisions `RESEND_API_KEY`) + redeploy → email live.
...
- [ ] **Contact email — accept Resend terms** at vercel.com → chanmekalas-projects → integrations → accept-terms/resend, then re-run `vercel integration add resend` (provisions `RESEND_API_KEY`) + redeploy → email notifications live. (Neon DB already done + verified; FormSubmit dropped — Cloudflare blocks Vercel IPs.) Later: verify a real domain in Resend to send from a branded address instead of `onboarding@resend.dev`. Still TODO: the auto-verify-against-pairing pipeline for song suggestions (Deezer/MusicBrainz check like `npm run enrich` — suggestions already arrive tagged Country×Genre).
