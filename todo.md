...
- [ ] **Contact destination** for the Contact popup (`components/Contact.tsx`). It's an in-app form now (note + song suggestion), but `send()` composes a `mailto` with a blank To: — give me the address (or a free email API / `/api/contact` provider) to turn it into a silent in-app submit. The "Add a song" suggestion arrives pre-tagged with Country×Genre; the auto-verify-against-pairing pipeline (Deezer/MusicBrainz check like `npm run enrich`) is still TODO.
