...
- [x] **Contact destination** — DONE 2026-07-20: `/api/contact` relays through FormSubmit to the private address (server-side only). **⚠️ Click FormSubmit's one-time activation link in the inbox** (a test note triggered it) or submissions won't deliver. Still TODO: the auto-verify-against-pairing pipeline for song suggestions (Deezer/MusicBrainz check like `npm run enrich` — suggestions already arrive tagged Country×Genre).
