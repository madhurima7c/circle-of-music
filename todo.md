...
**✉️ Contact fix — FormSubmit was silently failing**: it REQUIRES Origin/Referer headers, else HTTP 200 + `success:"false"` ("open through a web server") — so the old route's `res.ok` check passed while nothing sent, and no activation email ever fired. Route now sends the headers and reads the JSON `success` field. A direct test then got "This form needs Activation. We've sent you an email" → **activation email IS now in the destination inbox — click it once.**

**🗄️ Submissions database (Neon Postgres via Vercel Marketplace)**: new `lib/db.ts` — lazy `getSql()` (build-safe without DATABASE_URL), idempotent `CREATE TABLE IF NOT EXISTS submissions (id, kind note|song, subject, message, country, genre, song, created_at)`, `storeSubmission()` soft-fails so persistence never blocks the email. `/api/contact` now: store → email → 200 if EITHER worked (`{ok, stored, sent}`). Contact.tsx sends structured `{kind, country, genre, song}`. **⚠️ install blocked on user**: `vercel integration add neon` needs one-time marketplace-terms acceptance in the browser; after that, re-run the add (env vars auto-provision + redeploy) and the table self-creates on first submission.
- `.claude/launch.json`: added `circle-of-music-alt` (port 3010) for parallel-session dev servers (unused for now — Next 16 refuses two dev servers per dir anyway).
...
- [ ] **Contact — two one-time clicks**: (1) FormSubmit's activation link (REALLY sent 2026-07-20b after the Origin-header fix; check spam for formsubmit.co) or emails won't deliver; (2) Neon marketplace terms at vercel.com → chanmekalas-projects → integrations → accept-terms/neon, then `vercel integration add neon` finishes the submissions DB. Still TODO: the auto-verify-against-pairing pipeline for song suggestions (Deezer/MusicBrainz check like `npm run enrich` — suggestions already arrive tagged Country×Genre).
