...
- [ ] **Spotify client ID** — popup connect flow is BUILT (2026-07-16); user must create the free app at developer.spotify.com/dashboard, whitelist redirect URIs `http://127.0.0.1:3000/spotify-callback` (NOT localhost — Spotify rejects it for new apps) + `https://discovery-of-music.vercel.app/spotify-callback`, then set `NEXT_PUBLIC_SPOTIFY_CLIENT_ID` in `.env.local` AND Vercel env vars (redeploy). DONE 2026-07-16: client ID `1820…e296` set locally + Vercel production; live site shows Connect Spotify. Dev-mode cap for apps created after 2026-03-09: owner + **5** allowlisted users (Dashboard → User Management; was 25 before Spotify's Feb 2026 policy change), owner must have Premium. Extended quota (public access) now requires a registered business w/ 250k MAU — not viable; the scale path for full songs is the Spotify **Embed iframe** (no dev app, no allowlist, full tracks for logged-in Premium users). Full songs need Premium; misses auto-fall back to previews.
